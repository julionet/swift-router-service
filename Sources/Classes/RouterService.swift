...
public final class RouterService: RouterServiceProtocol, RouterServiceRegistrationProtocol {
    ...
    public func navigate(
        ...
        let destinationFeatureType = handler.destination(forRoute: route)
        let destinationFeature = destinationFeatureType.initialize(withStore: store)
        let destinaionViewController: UIViewController
        
        if destinationFeature.isEnabled() {
            destinaionViewController = destinationFeature.build(fromRoute: route)
        } else {
            ...
            destinaionViewController = fallbackDestinationFeature.build(fromRoute: route)
        }

        presentationStyle.present(
            viewController: destinaionViewController, 
